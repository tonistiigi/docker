...
func (p *v1Pusher) imageListForTag(imgID images.ID, dependenciesSeen map[layer.ID]*v1DependencyImage, referencedLayers *[]layer.Layer) (imageListForThisTag []v1Image, err error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to get top layer id from image: %v", err)
	}
...
func generateDependencyImages(l layer.Layer, dependenciesSeen map[layer.ID]*v1DependencyImage) (imageListForThisTag []v1Image, parent *v1DependencyImage, err error) {
	if l == nil {
		return nil, nil, nil
	}
	...
	if dependenciesSeen != nil {
		if dependencyImage, present := dependenciesSeen[l.ID()]; present {
	...
	if dependenciesSeen != nil {
		dependenciesSeen[l.ID()] = dependencyImage
	}
...
func (p *v1Pusher) pushImage(v1Image v1Image, ep string) (checksum string, err error) {
	...
	if err := p.v1IDService.Set(v1ID, p.repoInfo.Index.Name, l.ID()); err != nil {

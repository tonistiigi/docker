...
func (fsc *FSCache) SyncFrom(ctx context.Context, id RemoteIdentifier) (builder.Source, error) { // cacheOpt
	...
	fsc.mu.Unlock()
	sr, err, _ := fsc.g.Do(id.Key(), func() (interface{}, error) {
		var sr *cachedSourceRef
		sr, err := fsc.store.Get(id.Key())
		if err == nil {
			return sr, nil
		}
		...
		if sr == nil {
			var err error
			sr, err = fsc.store.New(id.Key(), sharedKey)
			if err != nil {
				return nil, errors.Wrap(err, "failed to create remote context")
			}
		}

		if err := syncFrom(ctx, sr, tr, id); err != nil {
			sr.Release()
			return nil, err
		}
		if err := sr.resetSize(-1); err != nil {
			return nil, err
		}
		return sr, nil
	})
...
func newFSCacheStore(opt Opt) (*fsCacheStore, error) {
	...
	t = time.AfterFunc(gcInterval, func() {
		if err := s.GC(); err != nil {
...
func (s *fsCacheStore) New(id, sk string) (*cachedSourceRef, error) {
	s.mu.Lock()
	defer s.mu.Unlock()

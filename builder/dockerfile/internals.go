...
func (b *Builder) calcCopyInfo(cmdName, origPath string, allowLocalDecompression, allowWildcards bool, contextID *int) ([]copyInfo, error) {
	...
	if contextID != nil && runtime.GOOS == "windows" {
		p := strings.ToLower(origPath)
		if origPath == "/" || strings.HasPrefix(p, "/windows") || strings.HasPrefix(p, "c:/windows") {
	...
	var err error
	if contextID != nil {
		remote, err = b.imageContexts.context(*contextID)
		if err != nil {
			return nil, err
		}
	}
	...
	if allowWildcards && containsWildcards(origPath) {
		...
		if err := filepath.Walk(remote.Root(), func(path string, info os.FileInfo, err error) error {
			...
			// a * in it
			subInfos, err := b.calcCopyInfo(cmdName, rel, allowLocalDecompression, false, contextID)
			if err != nil {
				return err
			}
	...
	if contextID != nil {
		b.imageContexts.setCache(*contextID, origPath, copyInfos[0].hash)
	}

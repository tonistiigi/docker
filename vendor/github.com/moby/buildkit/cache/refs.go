...
type Mountable interface {
	Mount(ctx context.Context, readonly bool) (snapshot.MountFactory, error)
}
...
type cacheRecord struct {
	...
	view      string
	viewMount snapshot.MountFactory
...
func (cr *cacheRecord) Mount(ctx context.Context, readonly bool) (snapshot.MountFactory, error) {
	cr.mu.Lock()
	defer cr.mu.Unlock()
...
func setReadonly(mounts snapshot.MountFactory) snapshot.MountFactory {
	return &readOnlyMounter{mounts}
}

type readOnlyMounter struct {
	snapshot.MountFactory
}

func (m *readOnlyMounter) Mount() ([]mount.Mount, func() error, error) {
	mounts, release, err := m.MountFactory.Mount()
	if err != nil {
		return nil, nil, err
	}
